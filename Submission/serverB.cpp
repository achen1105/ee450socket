...
{
	ofstream myfile;
	myfile.open("block2.txt", ios::app);
	myfile << "\n" + tLog;
}
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		else if (buf[0] == 'S' && buf[1] == 'T')
		{
			...
			string rankingB = checkStats(username);

			// send req info to serverM
			if ((numbytes = sendto(sockfd, rankingB.c_str(), strlen(rankingB.c_str()), 0,
					(struct sockaddr *) &servMaddr, sizeof(servMaddr))) == -1) 
			{
				perror("server B client socket: sendto");
				exit(1);
